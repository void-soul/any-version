...
pub fn project_preview_unmanage(id: String) -> Result<ManagePreview, String> {
    ...
    if is_simple {
        let steps = vec![super::types::ManageStep {
            ...
            description: "取消托管后，Kira 将不再管理该项目的缓存、代理和镜像配置".to_string(),
            target: id.clone(),
        }];
    ...
    if !managed_vars.is_empty() {
        steps.push(super::types::ManageStep {
            ...
            description: format!("清除 Kira 设置的 {} 个环境变量", managed_vars.len()),
    ...
    if !compat_vars.is_empty() {
        steps.push(super::types::ManageStep {
            ...
            description: format!("以下 {} 个兼容层变量保持不变（不属于 Kira 管理）: {}", compat_vars.len(), compat_vars.join(", ")),
            target: String::new(),
        });
    }
...
pub fn bootstrap_git_repo(path: String, cmd: String) -> Result<(), String> {
    ...
    // 防范命令行注入：禁止在初始化脚本参数中包含危险 Shell 多命令连接符
    if cmd_to_run.contains('&') || cmd_to_run.contains('|') || cmd_to_run.contains(';') || cmd_to_run.contains('>') || cmd_to_run.contains('<') {
        return Err("非法初始化脚本命令，禁止包含 Shell 管道或多命令分隔符".to_string());
    }
