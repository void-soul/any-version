        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 mt-1 max-w-sm">
              {status.installed
                ? `检测到本地已有安装${status.install_source ? `（来源: ${status.install_source}）` : ""}，托管后可统一管理版本、环境变量和缓存。`
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {status.managed ? (
              ...
              <button onClick={handleManage} disabled={managing} className="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-semibold shadow-lg shadow-blue-500/20 cursor-pointer transition-all flex items-center gap-1.5 hover:scale-[1.02] active:scale-[0.98]">
                {managing ? "托管中..." : "托管此项目"}
              </button>
            )}
          </div>
