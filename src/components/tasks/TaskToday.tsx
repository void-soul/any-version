    ...
    <div className="h-full overflow-y-auto p-6 space-y-4">
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-slate-900 border border-white/10 rounded-lg px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-amber-500"
        />
        <button
          onClick={quickAdd}
      ...
      {/* 收集箱 */}
      {showInbox && (
        <div className="p-3 rounded-xl bg-slate-900/40 border border-amber-500/20 space-y-2">
          <h4 className="text-[10px] font-bold text-amber-300 flex items-center gap-1.5">
            ...
            inbox.map((t) => (
              <div key={t.id} className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-2 py-1.5 rounded-lg bg-amber-500/15 border border-amber-500/30 text-[9px] text-amber-300 hover:bg-amber-500/25 cursor-pointer transition-all flex-shrink-0"
                >
                  排期
                </button>
              </div>
