...
function renderMarkdown(content: string): string {
  try {
    // 净化经 marked 生成的 HTML，防止技能文件中的恶意脚本造成 XSS / RCE
    return DOMPurify.sanitize(marked.parse(content) as string);
  } catch {
    // 解析失败回退为转义后的纯文本，避免未转义内容被当作 HTML 注入
    return content
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");
        ...
        <div className="p-4 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500">{t("skillmgr.filesCount", { count: skill.files.length })}</p>
            </div>
          </div>
          <button onClick={onClose} className="text-slate-500 hover:text-white cursor-pointer text-xs">{t("skillmgr.close")}</button>
        </div>
